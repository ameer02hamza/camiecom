    ...
    try {
      ...
      await shopifyFetch({
        ...
        variables: {
          ...
          customer: { acceptsMarketing: newVal },
        },
        cache: "no-store",
      });
  ...
  return (
    <div className="bg-brand-dark dark:bg-black py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {status !== "idle" && (
          <div
            className={`inline-flex items-center gap-2 text-sm px-5 py-3 rounded-card mb-6 ${
              status === "error"
        ...
        {isLoggedIn ? (
          ...
          /* ── FIXED: Safari/mobile input issue ── */
          <form
          ...
          >
            <input
              ...
              style={{ fontSize: '16px' }}
              className={[
                /* Fixed height — same on all browsers/Safari */
                "w-full sm:flex-1 sm:w-auto min-w-0 h-11",
                "px-4 py-0",
                /* Typography */
                "text-sm text-brand-light placeholder:text-brand-light/40",
                /* Background & border */
                ...
                /* Focus */
                "focus:outline-none focus:border-brand-warm",
                "disabled:opacity-50",
                /* Box model */
                "box-border",
              ].join(" ")}
            ...
            <button
              type="submit"
              disabled={loading || status === "subscribed"}
              className="h-11 px-6 bg-brand-warm text-white rounded-btn text-sm font-medium hover:opacity-90 transition-opacity flex-shrink-0 inline-flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
