...
function mapCart(cart: ShopifyCart) {
  ...
  // Shopify cartCreate returns /cart/c/TOKEN which shows storefront first.
  // Replace with /checkouts/cn/TOKEN to go DIRECTLY to checkout page.
  const rawUrl = cart.checkoutUrl ?? ''
  const checkoutUrl = rawUrl.includes('/cart/c/')
    ? rawUrl.replace('/cart/c/', '/checkouts/cn/')
    : rawUrl
