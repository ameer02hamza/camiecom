...
export default function ShopPage() {
  ...
  const fetchProducts = useCallback(
    async (sort: SortOption, cursor: string | null = null, append = false) => {
      ...
      try {
        ...
        const newProducts = data.products.edges.map((e) =>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      {/* Header */}
      <div className="mb-8">
        <p className="text-xs font-semibold tracking-label uppercase text-ink-2 dark:text-ink-dk2 mb-1">
          Browse
        </p>
      ...
      {activeFilters.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          {activeFilters.map((f) => (
            <span
              key={f}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-dark dark:bg-brand-light text-brand-light dark:text-brand-dark text-xs font-medium rounded-pill"
            >
              {f}
              <button
                onClick={() => {
          ...
          <button
            onClick={clearAll}
            className="text-xs text-ink-2 dark:text-ink-dk2 hover:text-brand-red transition-colors underline"
          >
            Clear all
          </button>
        </div>
            ...
            <div>
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={cn(
                    "w-full text-left px-3 py-2 rounded-btn text-sm transition-colors",
                ...
                {collections.map((col) => (
                  <button
                    key={col.id}
                    onClick={() => setSelectedCategory(col.handle)}
                    className={cn(
                      "w-full text-left px-3 py-2 rounded-btn text-sm transition-colors",
            ...
            <div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <div
                  ...
                  className={cn(
                    "w-10 h-5 rounded-pill transition-colors relative flex-shrink-0 cursor-pointer",
                    showSaleOnly
                      ? "bg-brand-sage"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-6 gap-4">
            ...
            >
              ...
              {activeFilters.length > 0 && (
                <span className="w-4 h-4 bg-brand-dark dark:bg-brand-light text-brand-light dark:text-brand-dark text-[10px] rounded-pill flex items-center justify-center">
                  {activeFilters.length}
                </span>
              )}
            </button>
            <div className="flex items-center gap-2 ml-auto">
              <span className="text-sm text-ink-2 dark:text-ink-dk2 hidden sm:block">
                Sort:
              </span>
              <div className="relative">
                ...
                <ChevronDown
                  size={14}
                  className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-ink-2 dark:text-ink-dk2"
                />
              </div>
            </div>
          </div>
