  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="text-center mb-14">
        <p className="text-xs font-semibold tracking-label uppercase text-ink-2 dark:text-ink-dk2 mb-2">Get in touch</p>
        <h1 className="font-display text-5xl tracking-heading">Contact Us</h1>
        <p className="text-ink-2 dark:text-ink-dk2 mt-3 max-w-md mx-auto leading-body">We'd love to hear from you. Send us a message and we'll respond within one business day.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="space-y-6">
          {[
            { icon: Mail, title: 'Email', body: 'hello@Camiecom-brand.com', sub: 'We reply within 24h' },
            { icon: MessageSquare, title: 'Live Chat', body: 'Available in-app', sub: 'Mon–Fri, 9am–6pm GMT' },
            { icon: Clock, title: 'Response Time', body: '< 24 hours', sub: 'Usually much faster' },
          ...
          <div className="border border-border-light dark:border-border-dark rounded-card p-5 mt-4">
            <p className="text-sm font-medium text-ink-1 dark:text-ink-dk1 mb-2">Common questions</p>
            {['Shipping & tracking', 'Returns & exchanges', 'Size guide', 'Care instructions'].map(q => (
              <button key={q} className="block text-sm text-ink-2 dark:text-ink-dk2 hover:text-brand-warm transition-colors py-1">{q} →</button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {sent ? (
            <div className="bg-surface-light dark:bg-surface-dark rounded-panel shadow-card p-12 text-center">
              ...
              <p className="text-sm text-ink-2 dark:text-ink-dk2">We'll get back to you at {form.email} within 24 hours.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="bg-surface-light dark:bg-surface-dark rounded-panel shadow-card p-8 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-ink-1 dark:text-ink-dk1 mb-1.5">Full Name</label>
                  <input required value={form.name} onChange={f('name')} placeholder="Hamza Morgan"
                ...
                <div>
                  ...
                  <input type="email" required value={form.email} onChange={f('email')} placeholder="your@camiecom.com"
                    className="w-full h-11 px-4 text-sm border border-border-light dark:border-border-dark rounded-btn bg-transparent focus:outline-none focus:border-brand-dark dark:focus:border-brand-light transition-colors" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-ink-1 dark:text-ink-dk1 mb-1.5">Subject</label>
              ...
              <div>
                <label className="block text-sm font-medium text-ink-1 dark:text-ink-dk1 mb-1.5">Message</label>
                <textarea required rows={5} value={form.message} onChange={f('message')} placeholder="Tell us how we can help…"
              ...
              <Button type="submit" fullWidth size="lg" loading={loading}>Send Message</Button>
            </form>
          )}
        </div>
      </div>
