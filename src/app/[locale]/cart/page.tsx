...
export default function CartPage() {
  ...
  const { items, totalAmount, totalQuantity, isLoading } = useAppSelector(
    (s) => s.cart,
  );
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between gap-3">
                  <Link
                    ...
                    className="font-medium text-ink-1 dark:text-ink-dk1 hover:text-brand-warm transition-colors line-clamp-2"
                  >
                    {item.title}
                  </Link>
                  <button
                    onClick={() => dispatch(shopifyRemoveFromCart(item.lineId))}
                    disabled={isLoading}
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  <span className="font-semibold text-ink-1 dark:text-ink-dk1">
          ...
          <div className="bg-surface-light dark:bg-surface-dark rounded-panel shadow-card p-7 sticky top-24">
            ...
            <div className="space-y-3 text-sm mb-6">
              ...
              <div className="flex justify-between text-ink-2 dark:text-ink-dk2">
                ...
                <span
                  className={
                    shipping === 0 ? "text-brand-sage font-medium" : ""
                  }
                >
              ...
              {shipping > 0 && (
                <div className="flex items-start gap-2 bg-brand-sage/10 rounded-btn p-3 mt-2">
                  <Truck
                    size={14}
                    className="text-brand-sage mt-0.5 flex-shrink-0"
                  />
            ...
            <div className="flex gap-2 mb-6">
              <input
                ...
                className="flex-1 h-10 px-3 text-sm border border-border-light dark:border-border-dark rounded-btn bg-transparent focus:outline-none focus:border-brand-dark dark:focus:border-brand-light"
              />
              <button className="h-10 px-4 text-sm font-medium border border-border-light dark:border-border-dark rounded-btn hover:bg-border-light dark:hover:bg-border-dark transition-colors">
                Apply
              </button>
            </div>
            ...
            <Link
              href="/shop"
              className="block text-center text-sm text-ink-2 dark:text-ink-dk2 hover:text-ink-1 dark:hover:text-ink-dk1 transition-colors mt-4"
            >
              Continue Shopping
            </Link>
          </div>
