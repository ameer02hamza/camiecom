...
export default function WishlistPage() {
  ...
  const handleMoveToCart = async (item: typeof items[0]) => {
    setAddingId(item.id)
    // variantId same as id for wishlist items saved from ProductCard
    ...
    if (shopifyAddToCart.fulfilled.match(result)) {
      ...
      dispatch(openCart())
      dispatch(addToast({ message: `${item.title} moved to cart`, type: 'success' }))
    } else {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="font-display text-3xl tracking-heading">Wishlist</h1>
        {items.length > 0 && (
          <span className="text-sm text-ink-2 dark:text-ink-dk2 ml-1">({items.length} items)</span>
        )}
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-24">
          ...
          <p className="text-lg text-ink-2 dark:text-ink-dk2 mb-2">Your wishlist is empty</p>
          <p className="text-sm text-ink-2 dark:text-ink-dk2 mb-6">Save items you love and come back to them later.</p>
