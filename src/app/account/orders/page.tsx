...
import {
  ArrowLeft, Package, Loader2, ChevronDown,
...
function OrderStepper({ financialStatus, fulfillmentStatus }: {
    ...
    <div className="px-6 py-5 border-t border-border-light dark:border-border-dark bg-bg-light/50 dark:bg-bg-dark/50">
      ...
      <div className="flex items-start gap-0">
        {steps.map((step, i) => {
          ...
          const dotColor =
            step.status === 'done'    ? 'bg-brand-warm text-white' :
          ...
          const labelColor =
            step.status === 'done'    ? 'text-ink-2 dark:text-ink-dk2' :
          ...
          const lineColor =
            step.status === 'done' ? 'bg-brand-warm' : 'bg-border-light dark:bg-border-dark'
          ...
          return (
            <div key={step.label} className={`flex flex-col items-center ${isLast ? 'flex-none' : 'flex-1'}`}>
              ...
              <div className="flex items-center w-full">
                ...
                {!isLast && (
                  <div className={`h-0.5 flex-1 mx-1 rounded-full transition-colors ${lineColor}`} />
                )}
              </div>

              {/* Label */}
              <p className={`text-[10px] mt-2 text-center leading-tight w-14 transition-colors ${labelColor}`}>
                {step.label}
              </p>
            </div>
          )
...
export default function OrdersPage() {
  ...
  useEffect(() => {
    if (!accessToken) { setLoading(false); setError('Please log in to view your orders.'); return }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-3xl tracking-heading">Order History</h1>
        </div>
        {!loading && orders.length > 0 && (
          <span className="text-sm text-ink-2 dark:text-ink-dk2">
            {orders.length}{hasNextPage ? '+' : ''} orders
          </span>
        )}
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          ...
          <p className="text-sm text-ink-2 dark:text-ink-dk2">Loading your orders...</p>
        </div>
      ) : error ? (
        <div className="text-center py-20 text-ink-2 dark:text-ink-dk2">
          <p className="text-lg mb-2">{error}</p>
          {!accessToken && (
            <Link href="/auth/login" className="text-brand-warm hover:underline text-sm">Log in →</Link>
          )}
        </div>
      ) : orders.length === 0 ? (
        <div className="text-center py-20">
          <Package size={40} className="mx-auto text-ink-2 dark:text-ink-dk2 opacity-30 mb-4" />
          <p className="text-lg text-ink-2 dark:text-ink-dk2 mb-2">No orders yet</p>
          <Link href="/shop" className="text-brand-warm hover:underline text-sm">Start shopping →</Link>
        </div>
      ) : (
        <>
          <div className="space-y-5">
            {orders.map(order => {
              ...
              return (
                <div key={order.id} className="bg-surface-light dark:bg-surface-dark rounded-card shadow-soft overflow-hidden">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-border-light dark:border-border-dark bg-bg-light dark:bg-bg-dark">
                    <div className="flex items-center gap-5">
                      <div>
                        <p className="text-xs text-ink-2 dark:text-ink-dk2 mb-0.5">Order</p>
                        <p className="font-semibold text-sm text-ink-1 dark:text-ink-dk1">#{order.orderNumber}</p>
                      </div>
                      <div>
                        <p className="text-xs text-ink-2 dark:text-ink-dk2 mb-0.5">Date</p>
                      ...
                      <div>
                        <p className="text-xs text-ink-2 dark:text-ink-dk2 mb-0.5">Total</p>
                        <p className="font-semibold text-sm text-ink-1 dark:text-ink-dk1">
                          {formatPrice(parseFloat(order.currentTotalPrice.amount))}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-ink-2 dark:text-ink-dk2 mb-0.5">Items</p>
                        <p className="text-sm text-ink-1 dark:text-ink-dk1">{itemCount}</p>
                      </div>
                    </div>
                    ...
                    {items.map((item, idx) => (
                      <div key={idx} className="flex items-center gap-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-ink-2 dark:text-ink-dk2">Qty {item.quantity}</p>
                        </div>
                        {item.variant && (
